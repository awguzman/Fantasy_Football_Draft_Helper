"""
This file constructs and trains a multi-agent Proximal Policy Optimization (PPO) algorithm to optimally draft a fantasy
football team.
...
class PPOAgent:
    ...
    def __init__(self, team_id, state_size, action_size, hidden_layers, position_limits, actor_max_norm=1.0,
                 critic_max_norm=1.0, clip_epsilon=0.15, actor_lr=1e-3, critic_lr=5e-4, discount_factor=0.95):
        ...
        self.clip_epsilon = clip_epsilon  # Objective clipping.
    ...
    def update_networks(self, debug=False):
        """Compute advantages and train actor-critic networks using PPO updates."""
        ...
        # Use actor network to compute probabilities
        ...
        actor_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.actor_network.parameters(), self.actor_max_norm)
        self.actor_optimizer.step()
        self.actor_scheduler.step()

        # Compute value predictions and target values
        values = self.critic_network(states)
        clipped_values = torch.clamp(values, values.detach() - self.clip_epsilon, values.detach() + self.clip_epsilon)

        # Update critic
        critic_loss = torch.max((values - returns).pow(2), (clipped_values - returns).pow(2)).mean()
        self.critic_optimizer.zero_grad()
        critic_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.critic_network.parameters(), self.critic_max_norm)
...
class FantasyDraft:
    ...
    def __init__(self, player_data, num_teams, num_rounds, state_size, action_size, hidden_layers, position_limits):
        ...
        self.position_limits = position_limits
        self.agents = [PPOAgent(team_id=i, state_size=state_size, action_size=action_size, hidden_layers=hidden_layers,
...
def run_training_routine():
    ...
    for agent in draft_simulator.agents:
        torch.save(agent.actor_network.state_dict(),
                   f"Trained_Agents/PPO_Agents/PPOAgent_{agent.team_id}_Actor_Network.pt")
